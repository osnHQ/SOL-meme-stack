...
export function getAdminAccount(PRIV_KEY:string|undefined){
  const secretKey = new Uint8Array(bs58.decode(PRIV_KEY));
  const keypair = Keypair.fromSecretKey(secretKey);
  return keypair
  // return {public_key:keypair.publicKey.toBase58(),secret_key:keypair.secretKey.toString()}
}


// export function getAdminAccountBIP39(mnemonic:string|undefined){
